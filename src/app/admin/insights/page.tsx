...
export default async function AdminInsightsPage() {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      description: "Inquiry records attached to a specific vehicle, energy item, or shop product.",
    },
    {
      label: "Search Events",
    ...
    },
    {
      ...
      description: "Products with at least one tracked view, favorite, build, or inquiry signal.",
    },
  ];
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-halo backdrop-blur-sm sm:p-8">
          ...
          <p className="mt-5 max-w-3xl text-sm leading-7 text-white/68 sm:text-base">
            This view keeps the analytics surface focused on the signals that already
            exist in the app: views, favorites, saved builds, inquiries, and search
            behavior. It is designed to help admins prioritize product decisions without
            turning the MVP into a BI suite.
          </p>
            ...
            {searchTrends.topQueries.length === 0 ? (
              <div className="rounded-[1.75rem] border border-dashed border-white/12 bg-black/24 p-6 text-center">
                ...
                <p className="mt-3 text-sm leading-6 text-white/62">
                  Search events are now tracked when users submit real catalog queries.
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-2 text-xs uppercase tracking-[0.22em] text-white/42">
                          {query.scopeLabel}
                          {query.topResultLabel ? ` · top results: ${query.topResultLabel}` : ""}
                        </p>
                      </div>
                      <p className="text-lg font-semibold text-white">{query.count}</p>
                    </div>
